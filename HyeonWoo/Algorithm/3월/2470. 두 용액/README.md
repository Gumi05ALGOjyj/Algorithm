...
1. 용액 리스트를 오름차순 정렬후 가장 왼쪽과 가장 오른쪽에 포인터를 둔다.
2. 두 포인터의 값의 합을 확인한다.
3. 두 포인터의 값이 최소 값이라면
  3.1. 합이 0이면 더 이상 볼 필요가 없으므로 종료한다.
  3.2. 합이 0보다 크면 더 작아저야 하므로 오른쪽 포인터를 왼쪽으로 한칸 이동한다.
  3.3. 합이 0보다 작으면 더 커져야 하므로 왼쪽 포인터를 오른쪽으로 한칸 이동한다.
4. 두 포인터가 만나기 전까지 반복한다.
