...
1. 석순과 종유석의 높이를 입력 받으며 각각 bottom, top 배열에 높이의 인덱스에 카운트 한다.
  1.1. 주의해야 할 점은 종유석은 거꾸로 있기 때문에 '전체높이 - 종유석 높이 + 1' 값의 인덱스에 카운트한다. -> top, bottom 배열의 길이를 h+1로 해줬기 때문
2. 전부 카운트 했으면 bottom은 위에서 아래로, top은 아래에서 위로 누적 합을 구한다.
3. bottom, top 배열의 각 인덱스의 합이 해당 layer에 존재하는 석순, 종유석의 개수이므로 순회하며 최소 개수를 구한다.
