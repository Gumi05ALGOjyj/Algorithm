...
● 단절점(정점을 제거한 경우에 그 정점을 포함한 그래프가 2개로 나누어지는 경우) 은 각 노드와 연결된 노드의 숫자를 ArrayList에 넣었고 찾고자 하는 정점과 연결된 정점이 한 개인 경우(리스트에 정점이 한 개 들어있는 경우) 그래프가 2개로 나누어진다.

● 단절선(간선을 제거했을 때, 간선이 포함된 그래프가 2개로 나누어지는 경우)은 간선을 지운 경우 무조건 그래프가 2개로 나누어진다. 
(why? 트리 형태의 그래프는 사이클이 존재하지 않으므로 간선을 지운 경우 무조건 그래프가 2로 나누어진다)
