...
### 문제 풀이
  <p>LRU는 가장 오래동안 참조되지 않은 페이지를 교체하는 방식이다.</p>
  <p>cache hit는 list값을 변경한다는 것이고, cache miss는 list값을 변경하지 않지만 참조는 하는 것이다.</p>
  <p>먼저 대소문자를 구별하지 않기 때문에 toUpperCase 함수를 이용한다.</p>
  <p>ArrayList를 이용하여 해당 city가 존재하면 list에서 city를 삭제한 후 바로 city를 추가한다.(cache miss)</p>
  <p>존재하지 않았을 때 list 사이즈가 캐시사이즈와 같다면 첫번째 인덱스를 삭제한다. 그리고 캐시 사이즈와 상관없이 무조건 list에 city를 추가한다.(cache hit)</p>
  
  ![image](https://user-images.githubusercontent.com/74286424/230804900-080d348d-88c5-4fa0-9ec8-897cb4be9e28.png)
