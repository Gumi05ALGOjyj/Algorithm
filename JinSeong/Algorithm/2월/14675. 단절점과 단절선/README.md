...
### 알고리즘 접근 방식
 <p>트리는 사이클이 없다는 점을 이용하여 간선을 제거하였을 때, 무조건 트리가 2개로 분리된 다는 것을 이용하여, t==2 일때 yes처리,</br>
 정점의 경우, 해당 정점이 두 개 이상의 정점과 이어져 있는 경우, 제거하였을 때, 트리가 분리된다는 것을 이용</p>
