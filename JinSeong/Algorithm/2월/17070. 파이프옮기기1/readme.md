...
### 풀이
 <p>DFS를 통해 X,Y,DIRECTION 파라미터로</br>
 switch문을 통해 direction을 받고 해당 direction의 다음 칸이 경계선 안에 있는지 && 벽이 없는지 확인 후 재귀</br>
 어떤 방향이든 대각선은 가기 때문에 switch 문 밖에서 대각선으로 가는 방향 재귀</br>
 좌표가 (N-1, N-1)로 잘 도착했다면 경우의 수 +1</p>
