...
### 알고리즘 접근 방식
 ...
 <p>트리의 루트가 고정되어 있으면 dfs를 이용해 내려가면서 탐색할 수 있지만,</br>
 아래에서 위로 올라가면서 부모를 찾는 방식을 이용</br>
 먼저, 해당 노드들의 깊이를 먼저 반환 받고</br>
 깊이의 차이가 다르면, 더 깊은 것을 올리면서 다른 노드와 깊이가 갖게 맞춘다.
 </br>깊이가 같은데 노드가 다른 것을 가리키고 있다면, 둘 다 부모를 찾아 올라가면서 겹칠 때 까지 올린다.</p>
