...
### 풀이
 ...
 <p>archer를 위로 올리는 방식</br>
 백트래킹을 통해 궁수의 위치 반환</br>
 궁수 3명의 위치가 정해지면 원래 상태의 map을 map2로 복사</br>
 for(궁수 3)안의 for(가로) 안의 for(세로)에서 배열이 1을 가리키고 있고, 사거리 안에 있으면 min계산/x,y좌표 저장</br>
 다음에 들어 왔을때, min 보다 작으면 min,x,y 다시 재정의</br>
 왼쪽에서 오른쪽으로 탐색하는 방향이기 때문에, 왼쪽에서 min을 설정해버리면, 가운데나 오른쪽이 같은거리에 있어도 방문X</br>
 배열이 0이 아닌경우만 방문, 적들을 찾으면 적들을 죽이면서 1->0 배열값 변경</br>
 배열이 0이 아닌경우만 방문하기 때문에, 다른 궁수가 같은 적을 죽여도 이미 0인걸 찾아서 temp가 추가되지 않음</br>
 모든 작업이 끝나면 궁수를 위로 올림
 </p>
