...
### 풀이
 <p>T는 5이하, S는 30이하로 들어온다는 점을 이용해서 T의 인덱스를 arraylist로 받는다.</br>
 장애물을 3개 둘 수 있으므로 백트래킹을 이용하여 cnt==3 일때 check</br>
 'X'를 만났을 때 'O'로 바꿔주며 백트래킹 시도</br>(시간을 줄이기 위해서 for문 시작점을 백트래킹 파라미터로 받았는데 시간이 줄어들지 않음... 이건 왜이럴까)</br></br>
 
 check부분 </br>
 선생님 한명씩 검사</
 dx와 dy 배열을 선언하고 한 방향으로 직진하며 4방향 모두 탐색</br>
 if) 직진하다가 벽만나면 break;</br>
 if) 'O'만나면 break;</br>
 if) 'S'만나면 탈출 실패-> false
 false를 안만나고 무사히 반복문을 마치면 true</p>
